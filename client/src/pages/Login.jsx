...
export default function Login() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      // 🔐 STORE TOKENS SEPARATELY (IMPORTANT FIX)
      if (data.user.role === "admin") {
        localStorage.setItem("adminToken", data.token);
        localStorage.removeItem("token"); // prevent conflict
      } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 p-4">
      ...
      >
        <h2 className="text-3xl font-bold text-center bg-gradient-to-r from-indigo-600 to-pink-500 bg-clip-text text-transparent mb-2 flex items-center justify-center gap-2">
          <LogIn size={26} /> Login
        </h2>
